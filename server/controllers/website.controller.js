...
export const generateWebsite = async (req, res) => {
  try {
    ...
    if (user.credits < 50)
      return res.status(400).json({ message: "credits are insufficient" });
...
export const getAll = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "getAll websites error" });
  }
};
...
export const getBySlug = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `getBySlug error ${error}` });
  }
};
